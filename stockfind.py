...
if start_btn:
    ...
    if not market_df.empty:
        ...
        st.success("✅ 분석 완료!")
# 분석 시작 버튼을 누르지 않았을 때 (필터링 등)의 로직
else:
    # 이전에 분석한 데이터가 있다면 필터링해서 보여줍니다.
...
# Outlook 버튼은 항상 표시되도록 로직의 맨 마지막에 둡니다.
# (단, 분석 데이터가 있을 경우에만 활성화된 링크가 보입니다)
if 'df_all' in st.session_state:
    df = st.session_state['df_all']
    # 현재 필터에 맞는 display_df를 다시 만듭니다.
    display_df = df.copy()
    if st.session_state.filter == "매수": display_df = df[df['상태'].str.contains("매수")]
    elif st.session_state.filter == "매도": display_df = df[df['상태'].str.contains("매도")]
