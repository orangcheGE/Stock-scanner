...
if st.sidebar.button("🚀 분석 시작"):
    ...
    st.rerun() # 버튼 클릭 상태를 즉시 반영하여 재실행

st.subheader("📊 진단 및 필터링")
c1, c2, c3 = st.columns(3); total_metric, buy_metric, sell_metric = c1.empty(), c2.empty(), c3.empty()
col1, col2, col3 = st.columns(3)
if col1.button("🔄 전체 보기", use_container_width=True): st.session_state.filter = "전체"; st.rerun()
if col2.button("🔴 매수 관련만", use_container_width=True): st.session_state.filter = "매수"; st.rerun()
if col3.button("🔵 매도 관련만", use_container_width=True): st.session_state.filter = "매도"; st.rerun()
...
if st.session_state.is_running:
    ...
    if not market_df.empty:
        ...
        for i, (idx, row) in enumerate(market_df.iterrows()):
            ...
            if res:
                results.append(res); df_all = pd.DataFrame(results, columns=['코드', '종목명', '등락률', '현재가', '이격률', '상태', '해석', '차트']); st.session_state['df_all'] = df_all
                total_metric.metric("전체 종목", f"{len(df_all)}개"); buy_metric.metric("매수 신호", f"{len(df_all[df_all['상태'].str.contains('매수')])}개"); sell_metric.metric("매도 신호", f"{len(df_all[df_all['상태'].str.contains('매도')])}개")
                result_title.subheader(f"🔍 결과 리스트 (실시간 분석 중...)"); main_result_area.write(df_all.style.applymap(lambda x: 'color: #ef5350; font-weight: bold' if '매수' in str(x) else ('color: #42a5f5' if '매도' in str(x) else ''), subset=['상태']).applymap(lambda x: 'color: #ef5350' if '+' in str(x) else ('color: #42a5f5' if '-' in str(x) else ''), subset=['등락률', '이격률']))
    ...
    if not display_df.empty: main_result_area.write(display_df.style.applymap(lambda x: 'color: #ef5350; font-weight: bold' if '매수' in str(x) else ('color: #42a5f5' if '매도' in str(x) else ''), subset=['상태']).applymap(lambda x: 'color: #ef5350' if '+' in str(x) else ('color: #42a5f5' if '-' in str(x) else ''), subset=['등락률', '이격률']))
    else: main_result_area.info(f"'{st.session_state.filter}' 조건에 맞는 데이터가 없습니다.")
    # 이메일 링크
    if not display_df.empty: st.markdown(f'<a href="mailto:?subject=주식리포트&body={urllib.parse.quote(display_df[["종목명", "현재가", "상태"]].to_string(index=False))}" ...>...</a>', unsafe_allow_html=True)
else:
    # --- 초기 상태 로직 ---
