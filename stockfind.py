...
def calc_signal_score(last, prev, ichimoku_status, rsi_val, cci_val, disparity, bw_series, price_col='종가'):
    """
    각 지표별 점수 합산 → 7단계 종합 신호 반환 (***로직 수정***)
    """
    score = 0
    detail = {}
    ...
    # ── 1. 일목균형표 (±3) ──────────────────
    # '구름대 위' 점수 제거, '최근 돌파'에만 점수 부여
    if '최근 상향돌파' in ichimoku_status:
        s = 3
    elif '구름대 위' in ichimoku_status: # 이제 점수 없음
        s = 0
    ...
    # ── 3. CCI (Commodity Channel Index) (±2) ───────
    # MACD처럼 방향 전환에 점수 부여 (***신규 추가***)
    cci_now = cci_val
    cci_prev = prev['CCI']
    if cci_now > 0 and cci_prev <= 0:
        s = 2    # 0선 상향 돌파 (강한 매수)
    elif cci_now > 0 and cci_now > cci_prev:
        s = 1    # 0선 위에서 상승 중
    elif cci_now < 0 and cci_prev >= 0:
        s = -2   # 0선 하향 돌파 (강한 매도)
    elif cci_now < 0 and cci_now < cci_prev:
        s = -1   # 0선 아래에서 하락 중
    else:
    ...
    # ── 4. 이동평균 정배열 (±2) ─────────────
    s = 0
    if price > last['60MA']: s += 1
    else: s -= 1
    if price > last['20MA']: s += 0.5
    else: s -= 0.5
    if last['5MA'] > last['20MA']: s += 0.5
    else: s -= 0.5
    s = round(s)
    score += s
    detail['MA'] = s
    ...
    is_high_disp    = disparity > 8  # 추세추종 기준을 8%로 약간 낮춤
    is_trending_up  = disparity > 5 and price > last['20MA'] # 20MA 위에서 5% 이상 상승
    is_oversold     = rsi_val < 45
    is_near_20ma    = abs(disparity) <= 4
    is_above_cloud  = '구름대 위' in ichimoku_status or '상향돌파' in ichimoku_status
    macd_golden     = hist_now > 0 and hist_prev <= 0
    cci_golden      = cci_now > 0 and cci_prev <= 0

    if (score >= 4 and is_above_cloud and is_near_20ma and is_oversold and (macd_golden or cci_golden)):
        signal = "🔥 적극매수"
    elif (score >= 3 and (is_overbought or is_high_disp)):
        signal = "🚀 추세추종"
    # ***'추세 상승중' 신호 추가***
    elif (score >= 1 and is_trending_up and not is_high_disp):
        signal = "⚡ 추세 상승중"
    elif score >= 2:
        signal = "📈 매수관심"
    elif (score >= -1 and is_above_cloud and is_near_20ma and rsi_val <= 55):
        signal = "🔄 눌림목대기"
    elif score >= -1:
        signal = "⏸️ 관망"
    elif score >= -4:
...
def analyze_stock(code, name, current_change):
    try:
        ...
        high_9  = df['고가'].rolling(9).max(); low_9   = df['저가'].rolling(9).min()
        df['tenkan_sen'] = (high_9 + low_9) / 2
        high_26 = df['고가'].rolling(26).max(); low_26  = df['저가'].rolling(26).min()
        df['kijun_sen'] = (high_26 + low_26) / 2
        high_52 = df['고가'].rolling(52).max(); low_52  = df['저가'].rolling(52).min()
        ...
        df['CCI'] = calc_cci(df['고가'], df['저가'], df['종가']) # *** CCI 추가 ***
        ...
        # 최근 3일(t-1, t-2, t-3) 중 하나라도 구름대 아래에 있다가 오늘 구름대 위로 올라온 경우
        for i in range(1, 4):
            if len(df_final) > i + 1:
                prev_day = df_final.iloc[-(i + 1)]
                prev_cloud_top = max(prev_day['senkou_a'], prev_day['senkou_b'])
                if prev_day['종가'] <= prev_cloud_top and price_today > cloud_top_today:
        ...
        for i in range(1, 4):
            if len(df_final) > i + 1:
                prev_day = df_final.iloc[-(i + 1)]
                prev_cloud_bot = min(prev_day['senkou_a'], prev_day['senkou_b'])
                if prev_day['종가'] >= prev_cloud_bot and price_today < cloud_bot_today:
                    is_breakdown = True
                    break

        if price_today > cloud_top_today:
            ichimoku_status = "🔥 최근 상향돌파" if is_breakthrough else "📈 구름대 위"
        elif price_today < cloud_bot_today:
            ichimoku_status = "🧊 최근 하향이탈" if is_breakdown else "📉 구름대 아래"
        else:
        ...
        score, signal, detail = calc_signal_score(
            last, prev, ichimoku_status, rsi_val, last['CCI'], disparity, df_final['BB_width']
        )
...
def style_signal(val):
    ...
    if '추세 상승중' in v: return 'color:white;background-color:#00897b;font-weight:bold' # 청록색
    if '추세추종'   in v: return 'color:white;background-color:#e65100;font-weight:bold'
    if '눌림목대기' in v: return 'color:white;background-color:#6a1b9a;font-weight:bold'
...
| 📈 매수관심 | 상승 초입, 아직 안 오른 상태 |
| ⚡ 추세 상승중 | 20MA 위, 추세 강하지만 과열 아님 |
| 🚀 추세추종 | 이미 많이 올랐지만 추세 강함 ⚠️ |
| 🔄 눌림목대기 | 구름대 위, 20MA 근처 반등 준비 |
| ⏸️ 관망 | 방향 불명확 |
| 📉 매도관심 | 하락 추세 |
| 🧊 적극매도 | 강한 하락 신호 |

> ⚠️ 추세추종은 이격률 8% 초과 또는 RSI 70 이상
...
trend_metric.metric("추세관련", "0개") # 추세추종 + 추세 상승중
...
if f2.button("🔥📈 매수", use_container_width=True): st.session_state.filter = "매수"
if f3.button("⚡🚀 추세",  use_container_width=True): st.session_state.filter = "추세"
if f4.button("🔄 눌림목",    use_container_width=True): st.session_state.filter = "눌림목"
if f5.button("📉🧊 매도", use_container_width=True): st.session_state.filter = "매도"
...
def update_metrics(df):
    ...
    buy_metric.metric("매수관심", f"{len(df[df['신호'].str.contains('적극매수|매수관심', regex=True)])}개")
    trend_metric.metric("추세관련", f"{len(df[df['신호'].str.contains('추세', regex=True)])}개")
    pullback_metric.metric("눌림목대기", f"{len(df[df['신호'].str.contains('눌림목대기')])}개")
    sell_metric.metric("매도관련", f"{len(df[df['신호'].str.contains('매도')])}개")

def apply_filter(df, f):
    if f == "매수":
        return df[df['신호'].str.contains("적극매수|매수관심", regex=True)]
    elif f == "추세":
        return df[df['신호'].str.contains("추세", regex=True)]
    elif f == "눌림목":
