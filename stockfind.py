...
def style_ichimoku_series(s: pd.Series):
    """일목균형표 컬럼(Series)에 스타일 적용"""
    styles = []
    for v in s:
        style = ''
        if '돌파' in v: style = 'color:white;background-color:#c62828;font-weight:bold'
        elif '위' in v: style = 'color:#ef5350'
        elif '아래' in v: style = 'color:#64b5f6'
...
if 'df_all' in st.session_state and not st.session_state['df_all'].empty:
    ...
    with result_area:
        ...
        # BUG-FIX: Styler를 더 안전하게 적용
        styler = df_to_show.style
        
        # 존재하는 컬럼에 대해서만 스타일 적용
        if '신호' in df_to_show.columns:
            styler = styler.map(style_signal, subset=['신호'])
        
        # apply를 사용하여 여러 컬럼에 안전하게 적용
        ichimoku_cols = [col for col in ['일목(일)', '일목(주)'] if col in df_to_show.columns]
        if ichimoku_cols:
            styler = styler.apply(style_ichimoku_series, subset=ichimoku_cols)

        if '20MA' in df_to_show.columns:
            styler = styler.map(style_ma20, subset=['20MA'])
        if '주간연속봉' in df_to_show.columns:
            styler = styler.map(style_weekly_consec, subset=['주간연속봉'])
        if '등락률' in df_to_show.columns:
            styler = styler.map(style_pct, subset=['등락률'])
