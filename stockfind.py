...
def analyze_stock(code, name, current_change):
    try:
        ...
        if (macd_hist_last > 0 and macd_hist_prev <= 0) or (price > ma20 and prev_price < prev_ma20): status, trend = "적극 매수", "🔥 엔진 점화 (강력 매수 신호)"
...
main_result_area = st.empty() # 결과를 표시할 컨테이너
    ...
    if not market_df.empty:
        ...
        for i, (idx, row) in enumerate(market_df.iterrows()):
            ...
            if res:
                ...
                # ▼▼▼▼▼ 핵심 수정 부분 ▼▼▼▼▼
                # with문을 사용하지 않고, 컨테이너에 직접 dataframe을 할당합니다.
                main_result_area.dataframe(
                    df_all.style.applymap(lambda x: 'color: #ef5350; font-weight: bold' if '매수' in str(x) else ('color: #42a5f5' if '매도' in str(x) else ''), subset=['상태'])
...
# --- 필터링 및 표시 로직 (분석이 끝난 후 또는 버튼 클릭 시) ---
# start_btn이 True가 아닐 때만 이 블록이 실행됩니다.
    ...
    # 여기도 마찬가지로 컨테이너에 직접 할당합니다.
    main_result_area.dataframe(
        display_df.style.applymap(lambda x: 'color: #ef5350; font-weight: bold' if '매수' in str(x) else ('color: #42a5f5' if '매도' in str(x) else ''), subset=['상태'])
                  .applymap(lambda x: 'color: #ef5350' if '+' in str(x) else ('color: #42a5f5' if '-' in str(x) else ''), subset=['등락률', '이격률']),
        use_container_width=True,
        column_config={"차트": st.column_config.LinkColumn("차트", display_text="열기"), "코드": st.column_config.TextColumn("코드", width="small")},
        hide_index=True
